...
def get_data_full(df_r, df_t, df_c):
    ...
    if not df_c_clean.empty:
        # Forza Anno a stringa pulita (senza .0)
...
with tab2:
    st.header("📊 Storico & Configurazioni")
    if not df_all.empty:
        ...
        st.metric(f"Risparmio {anno_sel}", f"{df_st['Risparmio'].sum():.2f} €")

    st.divider()
    
    # --- LOGICA SOSTITUZIONE BENZINA MIGLIORATA ---
    with st.expander("⛽ Imposta Prezzo Benzina per Anno"):
        ...
        if st.button("Salva Prezzo Anno"):
            # 1. Prepariamo il DataFrame esistente forzando tutti gli anni a stringa pulita
            ...
            # 2. Filtriamo via l'anno che vogliamo sovrascrivere
            df_filtered = df_config_clean[df_config_clean['Anno'] != str(target_year)]
            
            # 3. Aggiungiamo il nuovo dato
